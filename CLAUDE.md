...
Current state: Go reference at `v1.2.0`, self-contained (Blake3 + AES inlined from `StoaChain/Blake3` at v1.1.0), 40×40 bitmap input added as 6th key-gen path at v1.2.0, 105-vector test corpus (50 bitstring + 15 seed-words + 20 bitmap + 20 Schnorr). TypeScript port planned across 14 phases in `docs/TS_PORT_PLAN.md` v2 — next action is `Exec: begin Phase 0b` (TypeScript build scaffold).
