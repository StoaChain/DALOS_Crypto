// File CharacterMatrix.go — the Demiourgos address-derivation rune
// table and its accessor.
//
// HARDENING (v4.0.3, audit cycle 2026-05-04, F-LOW-012): split out of
// Elliptic/KeyGeneration.go where this 297-line static rune table
// dominated the file (~38% of total LoC). The TS port has the same
// split (`ts/src/gen1/character-matrix.ts` separate from `key-gen.ts`),
// so this file brings the Go-side structure into parity. No behavior
// change — same package (`Elliptic`), same exports (`CharacterMatrix`),
// same cache var (`characterMatrixCache`) populated at package init.
//
// The 256-entry rune table maps each possible Blake3-output byte
// (0-255) to a Unicode code point; ConvertToLetters in KeyGeneration.go
// indexes it by `value/16` and `value%16` to convert the 160-byte hash
// into the 160-rune Demiourgos address suffix that gets prefixed with
// `Ѻ.` (standard) or `Σ.` (smart) — see DalosAddressMaker.
//
// Genesis byte-identity: this file is byte-identical to lines 478-789
// of the pre-v4.0.3 KeyGeneration.go. Same runes in the same positions
// — every Ѻ./Σ. address in the 105-vector corpus regenerates exactly.
//
